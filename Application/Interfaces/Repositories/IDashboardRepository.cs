...
public interface IDashboardRepository
{
    ...
    Task<List<ActivityItemDto>> GetRecentActivityAsync(Guid businessId, int limit, DateTime? fromUtcInclusive, DateTime? toUtcExclusive);
    Task<List<TopProductRowDto>> GetTopProductsAsync(Guid businessId, int take, DateTime? fromUtcInclusive, DateTime? toUtcExclusive);
    Task<List<PendingOrderRowDto>> GetPendingOrdersWithBalanceAsync(Guid businessId);
    Task<ProfitBySourceDto> GetProfitBySourceAsync(Guid businessId, DateTime? fromUtcInclusive, DateTime? toUtcExclusive);
}
